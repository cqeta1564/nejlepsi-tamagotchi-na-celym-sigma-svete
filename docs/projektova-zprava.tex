...
\begin{itemize}[leftmargin=2em]
  ...
  \item Zaklad PWA assetu: \texttt{manifest.webmanifest}, ikony a generovany \texttt{sw.js}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \texttt{src/data} -- ukazkova data mazlicku a podpurne definice akci,
...
V repozitari jsou jeste starsi komponenty \texttt{ActionButton}, \texttt{ActionsPanel}, \texttt{PetName}, \texttt{ScreenHeader} a \texttt{StatBar}. Aktualni render strom je ale nepouziva. Hlavni obrazovky ted jedou pres \texttt{PetSelectionScreen}, \texttt{HomeScreen}, \texttt{StatsPanel}, \texttt{PetCard} a modal.
...
Zdroj puvodnich dat je v \texttt{src/data/pets.ts}, kde jsou definovani tri ukazkovi mazlicci: Bobek, Indicka pneumatika a Bojka. Mistnosti a realne efekty tlacitek jsou ted definovane primo v \texttt{App.tsx}. Aplikace pouziva ctyri mistnosti:
\begin{itemize}[leftmargin=2em]
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Nasledne se spocita novy stav statistik podle aktualni mistnosti.
...
Z aktualnich statistik se zaroven odvozuje nalada mazlicka. Podle nizkeho zdravi, energie, hladu nebo stesti se meni \texttt{mood} a kresleny vyraz v komponentne \texttt{PetFace}. Pokud ma mazlicek malo energie, \texttt{PetCard} zobrazi nad avatarem bublinu \texttt{zzz}.
...
Aktualni verze projektu ma pripravenou cast PWA infrastruktury, ale jeste nejde o plne aktivovanou PWA.
...
Soucasne ale chybi dve dulezite vazby:
\begin{itemize}[leftmargin=2em]
  \item \texttt{src/main.tsx} zatim neimportuje a nevola \texttt{registerServiceWorker()},
  \item \texttt{index.html} zatim neobsahuje HTML odkaz na manifest.
\end{itemize}

Prakticky to znamena, ze build uz umi pripravit manifestove assety a service worker soubor, ale prohlizec je v aktualnim zapojeni nemusi pouzit jako plnohodnotnou instalovatelnou PWA. Z pohledu Lighthouse by proto PWA audit pravdepodobne jeste nebyl ciste splneny. Pro dodelani by stacilo doplnit odkaz na manifest do HTML a skutecne zavolat registraci service workeru v produkcnim vstupu aplikace.

\placeholderfigure{Placeholder pro Lighthouse audit}{Doplnte screenshot z Chrome DevTools -> Lighthouse -> PWA.\\Ocekavany stav: projekt ma manifest a generovany \texttt{sw.js}, ale registrace a manifest link jeste nejsou plne zapojene.}

\section{Testovani}

Automaticke jednotkove ani integracni testy v repozitari momentalne nejsou. Overovani probiha hlavne rucne a doplnkove pres CI, ktere spousti lint a build.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Ocekavany vysledek: protoze PWA cast jeste neni plne zapojena, aplikace nemusi po refreshi spolehlive offline fungovat.
  \item Skutecny vysledek: offline provoz zatim neni spolehlive garantovany.
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Matyas Potock\'y (Role B -- logika): priprava datovych typu, vyberu mazlicka, spravy stavu, ukladani do \texttt{localStorage}, casove herni smycky, CI, branch protection, nasazeni na GitHub Pages a zaklad PWA vystupu.
  \item Obe role na sebe navazovaly: nejdriv vznikl wireframe a komponentovy zaklad, potom se doplnila herni logika, persistence a deploy, a nakonec se vzhled znovu doladil vcetne animaci a produkcniho buildu.
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Pozitivni je take to, ze projekt ma CI workflow, verejne nasazeni na GitHub Pages a aktualne prochazi lint i produkcnim buildem.
  \item Povedlo se doplnit plynuly prechod mezi mistnostmi a svetly/tmavy motiv.
\end{itemize}

\subsection*{Co bylo problematicke}

\begin{itemize}[leftmargin=2em]
  \item V repozitari zustaly starsi nepouzivane komponenty z drivejsi iterace, takze kodova zakladna neni uplne uklizena.
  \item PWA cast je jen castecne pripravena. Existuje manifest, ikony a generovani \texttt{sw.js}, ale registrace service workeru a odkaz na manifest zatim nejsou plne zapojene.
  \item Projekt zatim nema automaticke testy, takze spoleha hlavne na rucni kontrolu, lint, TypeScript a build.
  \item README obsahuje nektere starsi informace o architekture, ktere uz presne neodpovidaji aktualnimu stavu kodu.
\end{itemize}

\subsection*{Co by slo dale rozvijet}

\begin{itemize}[leftmargin=2em]
  \item Dodelat skutecnou PWA vrstvu: zapojit manifest v HTML, zavolat registraci service workeru, otestovat offline fallback a instalaci na plochu.
  \item Pridat automaticke testy, idealne jednotkove testy herni logiky a zakladni integracni testy hlavnich obrazovek.
  ...
  \item Uklidit starsi nepouzivane komponenty a sjednotit README s aktualni implementaci.
\end{itemize}
